...
class BCMNeuron:
    def __init__(self, lgn_l, lgn_r, patterns, p=2, tau_w=10, tau_theta=1, dt=0.01, eps=0,
                 c0=50, condition='NR', iterations=100000, ttx_in = 0.30, ttx_gone =0.30, tau_ttx=400,
                 contra_factor =1.4, we=1, wi=0.1):
        ...
        self.ttx_in = np.floor(ttx_in*iterations) # time of ttx injection
        self.ttx_gone = np.floor(ttx_gone*iterations) # time of ttx starts to wear out
        self.tau_ttx = tau_ttx
        self.contra_factor = contra_factor
        ...
        self.weights_ri = np.random.rand(1,self.lgn_r,)  # record matrix; each row is a weight vector for the iteration
    ...
    def train(self, da_l, da_r, da_li, da_ri, ds_l=0, ds_r=0, 
        ...
        al = 0.1 # noise scaler
        ar = 0.00001 # noise scaler
        bl = 0.00001 # noise scaler
        br = 0.00001 # noise scaler
        ...
        if (self.lgn_l != np.shape(da_l)[1]) or (self.lgn_r != np.shape(da_r)[1]):
            ...
            for i in range(self.iterations):
                ...
                # generate noise
                nl = al*np.random.rand(self.patterns,self.lgn_l)
                nr = ar*np.random.rand(self.patterns,self.lgn_r)
                nli = al*np.random.rand(self.patterns,self.lgn_l)
                nri = ar*np.random.rand(self.patterns,self.lgn_r)
                ...
                if i <= self.ttx_in:  
                   ...
                   cn = self.contra_factor*np.sum(self.ml * nl[k]) + np.sum(self.mr * nr[k])   # both eyes get noise
                   c = ca - cs + cn 
                   # Feedback loop
                   ci = ci + self.we*c
                   c = c - self.wi*ci
                   ...
                   self.ml = self.ml + (1/self.tau_w) * self.dt * (phi * (0*da_l[k] - 0*ds_l[k] + nl[k]) - self.eps*self.ml)  
                   # update right weights for BCM neuron      
                   self.mr = self.mr + (1/self.tau_w) * self.dt * (phi * (da_r[k] - ds_r[k] + nr[k]) - self.eps*self.mr) 
                   # Add responses to all orientation in the tuning curve matrix
                   self.tuning_curve_l = np.vstack((self.tuning_curve_l, self.contra_factor * np.sum((da_l - ds_l + nl) * self.ml, axis=1)))
                   self.tuning_curve_r = np.vstack((self.tuning_curve_r, np.sum((da_r - ds_r + nr) * self.mr, axis=1)))
                else:
                   # generate noise
                   nl = bl*np.random.rand(self.patterns,self.lgn_l)
                   nr = br*np.random.rand(self.patterns,self.lgn_r)
                   nli = bl*np.random.rand(self.patterns,self.lgn_l)
                   nri = br*np.random.rand(self.patterns,self.lgn_r)
                   # ttx_factor = np.exp((i-ttx_gone)*np.heaviside(ttx_gone-i,0))
                   #ttx_factor = (1- np.exp(self.dt*(1/self.tau_ttx)*(self.ttx_in-i)))
                   ttx_factor = np.heaviside(i-self.ttx_gone,0)*\
                                (1- np.exp(self.dt*(1/self.tau_ttx)*(self.ttx_gone-i)))
                   ...
                   phi = c*(c - self.theta)
                   # update left weights for BCM neuron
                   self.ml = self.ml + (1/self.tau_w) * self.dt * (phi * (da_l[k] - ds_l[k] + nl[k]) - self.eps*self.ml)  
                   # update right weights for BCM neuron   
                   self.mr = self.mr + (1/self.tau_w) * self.dt * (phi * ttx_factor * (da_r[k] - ds_r[k] + nr[k]) - self.eps*self.mr) 
                   # Add responses to all orientation in the tuning curve matrix
                   self.tuning_curve_l = np.vstack((self.tuning_curve_l, np.sum(self.contra_factor* (da_l - ds_l + nl) * self.ml, axis=1)))
                   # self.tuning_curve_r = np.vstack((self.tuning_curve_r, np.sum(np.heaviside(i-self.ttx_gone,0)*(da_r - ds_r + nr) * self.mr, axis=1)))
                   self.tuning_curve_r = np.vstack((self.tuning_curve_r, np.sum(ttx_factor *(da_r - ds_r + nr) * self.mr, axis=1)))
  
                # update left weights for inhibitory neuron
                self.mli = self.mli + (1/self.tau_w) * self.dt * (phii * (da_li[k] - ds_li[k] + nli[k]) - self.eps*self.mli)  
                # update right weights for inhibitory neuron      
                self.mri = self.mri + (1/self.tau_w) * self.dt * (phii * (da_ri[k] - ds_ri[k] + nri[k]) - self.eps*self.mri) 
                ...
                self.tuning_curve_li = np.vstack((self.tuning_curve_li, np.sum((da_li - ds_li + nli) * self.mli, axis=1)))
                self.tuning_curve_ri = np.vstack((self.tuning_curve_ri, np.sum((da_ri - ds_ri + nri) * self.mri, axis=1)))
    ...
    def plot_responses(self):
        ...
        plt.axis([0, self.dt*self.iterations, -0.2, 3.2])
        plt.legend()
        plt.show()

        

    def plot_2eyesb(self,k):
        (row_l, col_l) = np.shape(self.tuning_curve_l)
        (row_r, col_r) = np.shape(self.tuning_curve_r)
        x = list(range(0, self.iterations ))
        x= self.dt * np.array(x)  
        plt.plot(x, self.tuning_curve_r[:, k])
        plt.plot(x, self.tuning_curve_l[:, k])
        plt.xlabel('iterations')
        plt.ylabel('responses')
        plt.axis([0, self.dt*self.iterations, -0.1, 2.1])
        plt.legend()
        plt.show()
